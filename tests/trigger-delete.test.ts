...
describe('DELETE Triggers', () => {
  ...
  afterEach(async () => {
    ...
    try {
      await triggers!.dropTrigger('item', 'test_delete_trigger');
    } catch (error) {
  ...
  test('conditional DELETE trigger should only fire when condition is met', async () => {
    ...
    await triggers!
      ...
      .withTiming('AFTER')
      .onEvents('DELETE')
      .withTypedCondition(({ OLD }) => OLD.status === 'active')
      .executeFunction('delete_notify_func')
      .create();
  ...
  test('DELETE trigger with condition builder should work', async () => {
